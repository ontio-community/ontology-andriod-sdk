...
//	}
	...
	public long readVarInt(long max) throws IOException {
		long fb = (long) readByte() & 0xffL;

		long value = 0;
		if (fb == 0xFD) {

			value = (long) readShort() & (Short.MAX_VALUE * 2 + 1);
		} else if (fb == 0xFE) {

			value = (long) readInt() & (Integer.MAX_VALUE * 2 + 1);
		} else if (fb == 0xFF) {
			value = (long) readLong() & (Long.MAX_VALUE * 2 + 1);
		} else {

			value = fb;
		}
		if (value > (max & (Long.MAX_VALUE * 2 + 1))) {
			throw new IOException();
		}
		return value;
	}
