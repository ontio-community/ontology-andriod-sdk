...
public class JWTCredential {
    ...
    public JWTCredential (JWTHeader header, JWTPayload payload, Account signer) throws Exception {
        ...
        jws = Base64.encodeToString(sig, Base64.NO_WRAP);
    }

    private JWTCredential (String header, String payload, String jws) {
        this.jws = jws;
        byte[] decodedHeader = Base64.decode(header, Base64.NO_WRAP);
    ...
    public byte[] genNeedSignData () {
        String header = Base64.encodeToString(
                  JSON.toJSONString(this.header, SerializerFeature.MapSortField).getBytes(), Base64.NO_WRAP);
        String payload = Base64.encodeToString(
                  JSON.toJSONString(this.payload, SerializerFeature.MapSortField).getBytes(), Base64.NO_WRAP);
    ...
    public byte[] parseSignature () {
        return Base64.decode(this.jws, Base64.NO_WRAP);
    }
